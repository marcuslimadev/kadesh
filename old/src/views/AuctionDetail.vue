                  ...
                  <div class="flex-1">
                    ...
                    <div class="grid grid-cols-3 gap-4 mb-2">
                      <div>
                        ...
                        <div class="text-xl font-bold text-green-600">R$ {{ parseFloat(bid.amount || 0).toFixed(2) }}</div>
                      </div>
                      <div>
                        <div class="text-xs text-gray-600">Score Total</div>
                        <div class="text-xl font-bold text-primary-900">{{ parseFloat(bid.calculated_score || 0).toFixed(1) }}</div>
                      </div>
                      <div>
                        <div class="text-xs text-gray-600">Reputação</div>
                        <div class="text-xl font-bold text-blue-600">
                          {{ bid.provider_rating ? parseFloat(bid.provider_rating).toFixed(1) : '0.0' }} ⭐
                        </div>
                      </div>
                    </div>

                    <div v-if="bid.proposal" class="mt-3 pt-3 border-t border-gray-200">
                      <div class="text-xs text-gray-600 mb-1">Proposta:</div>
                      <p class="text-sm text-gray-700 line-clamp-2">{{ bid.proposal }}</p>
                    </div>
                  </div>
                </div>
              </div>
...
const handlePlaceBid = async () => {
  ...
  if (!amount || amount <= 0) {
    error.value = 'Digite um valor válido para o lance'
    return
  }
  
  if (!bidForm.value.proposal_text || bidForm.value.proposal_text.trim().length < 10) {
    error.value = 'Digite uma proposta com pelo menos 10 caracteres'
    return
  }
  ...
  try {
    ...
    error.value = err.response?.data?.error || 'Erro ao enviar lance. Tente novamente.'
