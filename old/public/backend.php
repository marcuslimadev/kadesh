...
function handleGetProjectBids($projectId) {
    ...
    // Verificar se projeto existe e buscar dados
    $stmt = $db->prepare('SELECT contractor_id, max_budget FROM projects WHERE id = ?');
    $stmt->execute([$projectId]);
    $project = $stmt->fetch();
    ...
    // Verificar permissão: contractor vê todas, provider vê apenas as suas
    $isContractor = ($project['contractor_id'] == $user['id']);
    $isProvider = ($user['user_type'] === 'provider');
    
    if (!$isContractor && !$isProvider) {
        http_response_code(403);
    ...
    // Se for provider, filtrar apenas suas propostas
    if ($isProvider && !$isContractor) {
        $sql .= ' AND b.provider_id = ?';
    }
    
    $sql .= ' ORDER BY b.amount ASC, b.created_at ASC';
    ...
    // Formatar dados e calcular score
    foreach ($bids as &$bid) {
        $bid['id'] = (int)$bid['id'];
        ...
        $maxBudget = (float)($project['max_budget'] ?? 5000);
        $priceScore = ($maxBudget - $bid['amount']) / $maxBudget * 100 * 0.7;
        $reputationScore = $bid['provider_rating'] * 20 * 0.3; // Máximo 5 estrelas
        $bid['calculated_score'] = max(0, $priceScore + $reputationScore);
