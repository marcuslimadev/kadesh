      ...
      <div v-if="!loading">
        ...
        <div v-if="activeTab === 'system'" class="bg-white rounded-xl shadow-md p-6">
          ...
          <div class="mb-8 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
            <h4 class="text-lg font-medium text-yellow-800 mb-2">⚠️ Manutenção do Banco de Dados</h4>
            <p class="text-sm text-yellow-700 mb-4">
              Use esta ferramenta se estiver enfrentando erros 500 ou problemas de tabelas faltando (ex: advertisements).
              Isso forçará a execução das migrações pendentes.
            </p>
            <button
              @click="runMigrations"
              :disabled="migrating"
              class="px-4 py-2 bg-yellow-600 text-white rounded hover:bg-yellow-700 disabled:opacity-50 flex items-center"
            >
              ...
              {{ migrating ? 'Executando Migrações...' : 'Reparar Banco de Dados (Executar Migrações)' }}
            </button>
          </div>
...
const runMigrations = async () => {
  if (!confirm('Isso tentará criar tabelas ausentes no banco de dados. Continuar?')) return
  ...
  try {
    ...
    if (response.data.success) {
      successMessage.value = '✅ Migrações executadas com sucesso! ' + response.data.logs.join('\n')
    } else {
      error.value = '⚠️ Migrações concluídas com avisos: ' + response.data.logs.join('\n')
    }
  } catch (err) {
    console.error('Error running migrations:', err)
    error.value = '❌ Erro ao executar migrações: ' + (err.response?.data?.error || err.message)
  } finally {
