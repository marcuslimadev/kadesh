<template>
  <div class="min-h-screen bg-neutral-900 py-8 px-4">
    ...
    <div v-if="loading" class="max-w-5xl mx-auto">
      <div class="bg-white rounded-3xl shadow-2xl p-12 text-center border-4 border-neutral-300">
    ...
    <div v-else-if="!project" class="max-w-5xl mx-auto">
      <div class="bg-neutral-500 rounded-3xl shadow-2xl p-12 text-center border-4 border-neutral-300">
        <div class="relative inline-block mb-6">
        ...
        <router-link to="/projects" class="inline-flex items-center gap-2 bg-neutral-900 hover:bg-neutral-800 text-white px-8 py-4 rounded-2xl font-bold text-lg shadow-2xl hover:shadow-neutral-500/50 transform hover:scale-105 transition-all">
    ...
    <div v-else class="max-w-5xl mx-auto space-y-8">
      ...
      <!-- Main Card -->
      <div class="bg-white rounded-3xl shadow-2xl overflow-hidden border-4 border-transparent hover:border-neutral-300 transition-all">
        <!-- Gradient Top Bar -->
        ...
        <div class="p-8 sm:p-10">
          ...
          <!-- Title -->
          <h1 class="text-3xl sm:text-4xl md:text-5xl font-black text-transparent bg-clip-text bg-neutral-900 mb-6 leading-tight">
            {{ project.title }}
          </h1>
          ...
          <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <!-- Budget Card -->
            <div class="group bg-neutral-800 rounded-2xl p-6 shadow-xl hover:shadow-2xl transform hover:-translate-y-2 transition-all duration-300">
            ...
            <!-- Deadline Card -->
            <div class="group bg-neutral-700 rounded-2xl p-6 shadow-xl hover:shadow-2xl transform hover:-translate-y-2 transition-all duration-300">
              <div class="text-white text-center">
                <div class="text-4xl mb-2 group-hover:scale-125 transition-transform">📅</div>
            ...
            <!-- Bidding Ends Card -->
            <div class="group bg-neutral-600 rounded-2xl p-6 shadow-xl hover:shadow-2xl transform hover:-translate-y-2 transition-all duration-300">
              <div class="text-white text-center">
                <div class="text-4xl mb-2 group-hover:scale-125 transition-transform">⏰</div>
          ...
          <div v-if="project.required_skills && project.required_skills.length" class="mb-6">
            <div class="flex items-center gap-3 mb-3">
              <span class="text-lg font-black text-gray-700">🛠️ Habilidades Necessárias:</span>
            </div>
            <div class="flex flex-wrap gap-3">
              <span v-for="(skill, i) in project.required_skills" :key="i" 
                    class="bg-neutral-900 text-white text-sm font-bold px-5 py-2 rounded-full shadow-lg hover:shadow-xl transform hover:scale-110 transition-all">
                {{ skill }}
              </span>
            </div>
          </div>
      ...
      <div class="bg-white rounded-3xl shadow-2xl overflow-hidden border-4 border-neutral-300">
        <!-- Header -->
        ...
        <div class="p-6 sm:p-8">
          ...
          <div v-if="bids.length === 0" class="text-center py-16 bg-gradient-to-br from-gray-50 to-slate-50 rounded-2xl border-4 border-dashed border-gray-300">
            <div class="relative inline-block mb-6">
              ...
              <div class="absolute -top-2 -right-2 text-5xl animate-bounce">💡</div>
            </div>
          ...
          <div v-else class="space-y-6">
            <div v-for="bid in bids" :key="bid.id" 
                 class="group bg-white rounded-2xl shadow-lg hover:shadow-2xl border-4 border-transparent hover:border-neutral-300 transition-all duration-300 overflow-hidden">
              <!-- Gradient Top Bar -->
                ...
                <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
                  <div class="flex-1">
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-6">
                      <div class="bg-neutral-800 rounded-2xl p-5 text-white text-center shadow-lg">
                      ...
                      <div class="bg-neutral-700 rounded-2xl p-5 text-white text-center shadow-lg">
                  ...
                  <div v-if="canConfirmWinner && project.status === 'bidding'" class="lg:ml-6">
                    <button @click="confirmWinner(bid.id)" 
                            class="w-full lg:w-auto bg-neutral-800 hover:bg-neutral-700 text-white font-black px-8 py-4 rounded-2xl shadow-2xl hover:shadow-neutral-500/50 transform hover:scale-110 transition-all text-lg">
                      ✅ Aceitar Proposta
                    </button>
                  </div>
                </div>
      ...
      <div v-if="canBid && project.status === 'bidding'" class="bg-white rounded-3xl shadow-2xl overflow-hidden border-4 border-neutral-300">
        <!-- Header -->
        <div class="bg-neutral-900 p-6">
          <div class="flex items-center gap-4">
            <div class="w-14 h-14 bg-white/20 rounded-2xl flex items-center justify-center">
        ...
        <form @submit.prevent="submitBid" class="p-6 sm:p-8 space-y-6">
          <div class="grid sm:grid-cols-2 gap-6">
            <div>
              <label class="block text-sm font-black text-gray-700 uppercase tracking-wider mb-3">💰 Valor da Proposta (R$)</label>
            ...
            <div>
              <label class="block text-sm font-black text-gray-700 uppercase tracking-wider mb-3">⏱️ Prazo de Entrega (dias)</label>
          ...
          <div>
            <label class="block text-sm font-black text-gray-700 uppercase tracking-wider mb-3">📝 Detalhes da Proposta</label>
          ...
          <!-- Error Message -->
          <div v-if="bidError" class="bg-neutral-500 border-4 border-neutral-300 rounded-2xl p-6">
          ...
          <button :disabled="sendingBid" type="submit"
                  class="w-full bg-neutral-900 hover:bg-neutral-800 disabled:opacity-50 disabled:cursor-not-allowed text-white font-black text-xl px-8 py-5 rounded-2xl shadow-2xl hover:shadow-neutral-500/50 transform hover:scale-105 active:scale-95 transition-all">
      ...
      <div v-else-if="!user && project.status === 'bidding'" class="bg-gradient-to-br from-neutral-50 to-neutral-100 rounded-3xl shadow-2xl p-10 text-center border-4 border-neutral-300">
        <div class="relative inline-block mb-6">
        ...
        <h3 class="text-3xl font-black text-gray-900 mb-4">Faça login para enviar propostas!</h3>
        ...
        <div class="flex flex-col sm:flex-row gap-4 justify-center">
          <router-link to="/login" class="inline-flex items-center justify-center gap-2 bg-neutral-900 hover:bg-neutral-800 text-white font-black px-8 py-4 rounded-2xl shadow-2xl hover:shadow-neutral-500/50 transform hover:scale-110 transition-all text-lg">
            🔐 Fazer Login
          </router-link>
          <router-link to="/register" class="inline-flex items-center justify-center gap-2 bg-white text-neutral-900 font-black px-8 py-4 rounded-2xl shadow-2xl hover:shadow-xl transform hover:scale-110 transition-all border-4 border-neutral-300 text-lg">
            ✨ Criar Conta Grátis
          </router-link>
        </div>
      </div>
