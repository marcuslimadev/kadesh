    ...
    <div class="max-w-7xl mx-auto p-4 md:p-8 space-y-6">
      ...
      <div class="bg-white rounded shadow-primary p-6">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            <label class="block text-sm font-bold text-secondary-700 mb-2"> Total</label>
            <div class="text-2xl font-semibold text-primary-900">{{ projects.length }} projeto(s)</div>
          </div>
        </div>
      </div>
      ...
      <div v-if="loading" class="text-center py-20">
        ...
        <p class="mt-4 text-secondary-600 font-bold">Carregando projetos...</p>
      </div>
      ...
      <div v-else class="grid gap-4">
          ...
          <div class="flex flex-col md:flex-row justify-between gap-4">
            ...
            <div class="flex-1">
              <div class="flex items-start justify-between mb-3">
                <div class="flex-1">
                  ...
                  <p class="text-secondary-600 text-sm">{{ project.contractor_name }} - {{ project.contractor_email }}</p>
                </div>
              ...
              <p class="text-secondary-700 mb-4 line-clamp-2">{{ project.description }}</p>

              <!-- Estatísticas do Projeto -->
              <div class="grid grid-cols-4 gap-4 pt-4 border-t border-neutral-200">
                <div class="text-center">
                  <div class="text-lg font-semibold text-primary-900">R$ {{ formatMoney(project.max_budget) }}</div>
                  <div class="text-xs text-secondary-600 font-semibold">Orçamento Max</div>
                </div>
                <div class="text-center">
                  <div class="text-lg font-semibold text-primary-900">{{ project.bids_count || 0 }}</div>
                ...
                <div class="text-center">
                  ...
                  <div class="text-xs text-secondary-600 font-semibold">Menor Lance</div>
                </div>
                <div class="text-center">
                  <div class="text-lg font-semibold text-orange-600">{{ formatDate(project.created_at) }}</div>
                  <div class="text-xs text-secondary-600 font-semibold">Criado</div>
                </div>
              </div>
            </div>
            ...
            <div class="flex flex-col gap-2 min-w-[180px]">
              <router-link
                ...
                class="bg-primary-500 hover:bg-primary-600 text-white px-4 py-2 rounded font-bold transition-colors text-center flex items-center justify-center gap-2"
              >
                👁️ Ver Projeto
              </router-link>
              <button
        ...
        <div v-if="projects.length === 0" class="text-center py-20">
          ...
          <p class="text-xl text-secondary-600 font-bold">Nenhum projeto encontrado</p>
        </div>
      </div>
    </div>
  </div>
